...
view_mode = "compleet"; // [compleet, compleet_met_pocketholes_en_schroeven, compleet_gemonteerd, exploded, hout_exploded, voorzijde_met_lijst, eiken_lijst, plexiglas, epdm, stelplaatjes, houten_voetjes, houten_voetje, hout_voorplaat, hout_achterplaat, hout_zijwand_links, hout_zijwand_rechts, hout_bovenkap, hout_tussenbodem, hout_onderbodem, hout_schot_links, hout_schot_rechts, handgreep, print_kunststof_inzet_links, print_kunststof_inzet_rechts, print_hoekbeugel_links_voor, print_hoekbeugel_links_achter, print_hoekbeugel_rechts_voor, print_hoekbeugel_rechts_achter, print_hoekbeugels_set, rubberen_voet, rubberen_voeten, kunststof_middenvoetjes, compleet_zonder_aluminium, aluminium_frame, led_panelen, aluminium_frame_met_led_panelen, bovenhoekgrepen, print_bovenhoekgreep_links, print_bovenhoekgreep_rechts, compleet_voorplaat, compleet_grondplaat, klemmen_onder, klemmen_boven, klemmen_beide, glaslat_beschermprofiel_links, glaslat_beschermprofiel_rechts]
...
// Kunststof bovenhoekgrepen, vereenvoudigd naar de eerdere uitvoering.
bovenhoekgreep_steunhoogte = 20;
bovenhoekgreep_buitenwand_dikte = 4;
bovenhoekgreep_onderlip_extra = 12;
bovenhoekgreep_toplip_dikte = 3;
bovenhoekgreep_afronding = 6;
// Details uit de oorspronkelijke hoekkapconstructie.
bovenhoekgreep_onderband_hoogte = 12;
bovenhoekgreep_onderband_overlap = 4;
bovenhoekgreep_onderrand_radius = 2;
bovenhoekgreep_overgang_start_z = 9;
bovenhoekgreep_overgang_maat = 9;
bovenhoekgreep_overgang_overlap = 0.4;
bovenhoekgreep_boutgat_d = 4.3;
bovenhoekgreep_bout_y = [
    bovenkap_overstek_voor + 22,
    bovenkap_diepte - bovenkap_overstek_achter - 22
];
kleur_bovenhoekgreep = [0.18,0.72,0.24,1];
toon_bovenhoekgrepen = true;
bovenhoekgreep_explode_x = 35;
...
/* [Handgreep] */
handgreep_totale_breedte = 150;
handgreep_voorflens_breedte = handgreep_totale_breedte;
handgreep_gat_hartafstand = 132.4;
handgreep_montagegat_d = 5;
handgreep_bovenflens_diepte = 57.2;
handgreep_achterflens_hoogte = 57.2;
handgreep_materiaal_dikte = 1.5;

handgreep_opening_voor_breedte = 104;
handgreep_kom_diepte = 35;
handgreep_kom_wand = 4;
...
assert(bovenhoekgreep_steunhoogte == 20,
    "Het steunblok onder het bovenoverstek moet 20 mm hoog zijn.");
assert(bovenhoekgreep_onderlip_extra == 12,
    "De buitenwang moet 12 mm onder het steunblok uitsteken.");
assert(bovenhoekgreep_overgang_start_z == 9,
    "De 45 graden overgang moet 9 mm boven de onderzijde beginnen.");
assert(bovenhoekgreep_overgang_maat == 9,
    "De bovenste en onderste verstevigingshoeken moeten 9 x 9 mm zijn.");
...
hout_explode_x = 130;
hout_explode_z = 95;
handgreep_explode_y = 55;
handgreep_explode_z = 35;
...
// Alle oude groeven, montagegaten en serviceopeningen zijn verwijderd.
// ----------------------------------------------------------------------------
module houten_zijwand(rechts=false) {
    ...
    difference() {
        translate([x0,kast_y_voor,hout_buiten_z_min])
            cube([hout_dikte,kast_diepte,zijwand_hoogte]);
...
// ----------------------------------------------------------------------------
// Complete hoekhandgreep uit de oorspronkelijke versie.
// ----------------------------------------------------------------------------
// Center-handle geometry moved to components/top_center_handle.scad.

module handgreep_op_bovenkap() {
    xhart = (hout_buiten_x_min+hout_buiten_x_max)/2;
    bovenkap_achter_y =
        kast_y_voor
        - bovenkap_overstek_voor
        + bovenkap_diepte;

    // De verticale flens staat net buiten de houten achterrand.
    translate([
        xhart,
        bovenkap_achter_y + handgreep_materiaal_dikte,
        vrije_bovenbouw_hoogte + hout_dikte
    ])
        top_center_handle();
}
...
// ----------------------------------------------------------------------------
// Top side handles.
// Geometry moved to components/top_handles.scad. This legacy assembly wrapper
// only supplies the mounting positions relative to the wooden enclosure.
// ----------------------------------------------------------------------------
module bovenhoekgrepen(exploded=false) {
    dx = exploded ? bovenhoekgreep_explode_x : 0;
    kap_y0 = kast_y_voor-bovenkap_overstek_voor;

    translate([hout_buiten_x_min-dx,kap_y0,vrije_bovenbouw_hoogte])
        top_side_handle_oriented("left","model",true);

    translate([hout_buiten_x_max+dx,kap_y0,vrije_bovenbouw_hoogte])
        top_side_handle_oriented("right","model",true);
}
...
// ----------------------------------------------------------------------------
// Triangular prism across the full Y depth.
// Restored from the proven v202 source during the v1.0 repository cleanup.
module bovenhoekgreep_driehoek_prisma_y(p0,p1,p2,y0,diepte) {
    polyhedron(
        points=[
            [p0[0],y0,p0[1]],
            [p1[0],y0,p1[1]],
...
module houten_bovenkap() {
    ...
    handgreep_xhart = (hout_buiten_x_min+hout_buiten_x_max)/2;
    handgreep_bak_buiten_breedte =
        handgreep_opening_voor_breedte + 2*handgreep_kom_wand;
    handgreep_bak_buiten_diepte = handgreep_kom_diepte;
    handgreep_yhart =
        bovenkap_achter_y - handgreep_bovenflens_diepte/2;

    color(kleur_hout_kop)
    difference() {
        // Rechthoekige dakplaat met overstek aan voor- en achterzijde.
        translate([kap_x0,kap_y0,vrije_bovenbouw_hoogte])
            cube([
                bovenkap_totale_breedte,
                bovenkap_diepte,
                hout_dikte
        ...
        // 45° uitsparingen voor de kunststof eindgrepen.
        // De schuinte begint 9 mm boven de onderzijde van de 18 mm plaat.
        kap_eind_x0 = kap_x0;
        kap_eind_x1 = kap_x0+bovenkap_totale_breedte;
        kap_overgang_z0 =
            vrije_bovenbouw_hoogte+bovenhoekgreep_overgang_start_z;
        kap_overgang_z1 =
            vrije_bovenbouw_hoogte+hout_dikte;

        bovenhoekgreep_driehoek_prisma_y(
            [kap_eind_x0,kap_overgang_z0],
            [kap_eind_x0+bovenhoekgreep_overgang_maat,kap_overgang_z1],
            [kap_eind_x0,kap_overgang_z1],
            kap_y0-0.1,
            bovenkap_diepte+0.2
        );

        bovenhoekgreep_driehoek_prisma_y(
            [kap_eind_x1,kap_overgang_z0],
            [kap_eind_x1,kap_overgang_z1],
            [kap_eind_x1-bovenhoekgreep_overgang_maat,kap_overgang_z1],
            kap_y0-0.1,
            bovenkap_diepte+0.2
        );
        ...
        // U-vormige opening voor de handgreepbak.
        translate([
            handgreep_xhart-handgreep_bak_buiten_breedte/2,
            bovenkap_achter_y-handgreep_bak_buiten_diepte,
            vrije_bovenbouw_hoogte-1
        ])
            cube([
                handgreep_bak_buiten_breedte,
                handgreep_bak_buiten_diepte+1,
                hout_dikte+2
            ]);

        // Drie montagegaten in de bovenplaat voor de handgreep zelf.
        for(dx=[-handgreep_gat_hartafstand/2,0,handgreep_gat_hartafstand/2])
            translate([
                handgreep_xhart+dx,
                handgreep_yhart,
                vrije_bovenbouw_hoogte-1
            ])
                cylinder(h=hout_dikte+2,d=handgreep_montagegat_d,$fn=32);
    }
}
...
module houten_frame_assemblage(
    ...
    if(toon_bovenkap)
        translate([0,0,exploded ? hout_explode_z*1.35 : 0]) {
            ...
            if(toon_bovenhoekgrepen)
                bovenhoekgrepen(exploded);

            if(toon_handgreep)
                translate([
                    0,
                    exploded ? handgreep_explode_y : 0,
                    exploded ? handgreep_explode_z : 0
                ])
                    handgreep_op_bovenkap();
        }
